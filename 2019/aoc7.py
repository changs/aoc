    ...
    def run(self, program, input):
        ...
        while self.code[self.ip] != 99:
            ...
            if instr == 1: # add
                ...
                self.code[self.code[self.ip + 1]] = input.pop(0)
                self.ip += 2
            elif instr == 4: # output
                if mode_arg_1:
...
if __name__ == '__main__':
    with open('./aoc7.in', 'r') as content_file:
        content = content_file.read()
        cpu = CPU()
        phases = list(itertools.permutations('01234', 5))
        outputs = []
        for phase in phases:
            amp1 = cpu.run(content, [int(phase[0]), 0])
            amp2 = cpu.run(content, [int(phase[1]), int(amp1)])
            amp3 = cpu.run(content, [int(phase[2]), int(amp2)])
            amp4 = cpu.run(content, [int(phase[3]), int(amp3)])
            amp5 = cpu.run(content, [int(phase[4]), int(amp4)])
            outputs.append(int(amp5))
